...
struct MonteCarloResult {
    double price{};                  ///< Mean discounted payoff.
    double price_standard_error{};   ///< Standard error of that mean. Never hidden (I20).
    double delta{};                  ///< Pathwise dV/dS, per 1.00 of spot.
...
/// `S exp((r - q - sigma^2/2) T + sigma sqrt(T) z)` — the exact terminal spot
/// for one standard normal `z`. Exact in the sense that matters: the law of the
/// result is the law of `S_T`, with no discretisation between them.
[[nodiscard]] double terminal_spot_exact(const BlackScholesMarket& market,
                                         double expiry_years,
                                         double z) noexcept;

/// Euler-Maruyama over `normals.size()` equal steps of `T / n`.
///
/// The result may be negative, and is not clipped. A step multiplies by
/// `1 + (r - q) dt + sigma sqrt(dt) Z`, which is negative whenever `Z` is far
/// enough below zero, and a scheme that quietly floored it at zero would
/// converge at a different rate than the one the tests claim. The payoff
/// handles a negative spot correctly, so the honest thing is to let it through.
...
/// undiscounted: `w 1{w (S_T - K) > 0} S_T / S`.
///
